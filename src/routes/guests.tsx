...
export const Route = createFileRoute("/guests")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Add recipients, send invites and track delivery for every gift.",
      },
    ],
  }),
...
function GuestManager() {
  ...
  const toggleAll = () =>
    setSelected(
      allSelected
        ? selected.filter((id) => !filtered.some((g) => g.id === id))
        : Array.from(new Set([...selected, ...filtered.map((g) => g.id)])),
    );
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 pt-8 pb-28 sm:py-8 sm:px-6">
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <select
          value={rsvpF}
          onChange={(e) => setRsvpF(e.target.value as "all" | Guest["rsvp"])}
        ...
        <select
          value={invF}
          onChange={(e) => setInvF(e.target.value as "all" | Guest["invite"])}
          className="rounded-full border border-border bg-card px-4 py-2.5 text-sm"
        >
        ...
        {selected.length > 0 && (
          <button
            onClick={() => setShowInvite({ ids: selected })}
            className="inline-flex items-center gap-1.5 rounded-full bg-accent px-4 py-2.5 text-sm font-medium text-accent-foreground hover:opacity-90"
          >
            ...
            <tbody>
              {filtered.map((g) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <Badge cls={RSVP_BADGE[g.rsvp]}>{g.rsvp}</Badge>
                  </td>
                  <td className="px-4 py-3">
                    <Badge cls={INVITE_BADGE[g.invite]}>{g.invite}</Badge>
                  </td>
                  <td className="px-4 py-3">
                    <Badge cls={DELIV_BADGE[g.delivery]}>{g.delivery}</Badge>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-1">
                      <button
                        onClick={() => setShowInvite({ ids: [g.id] })}
                        title="Send invite"
                        className="rounded-full p-2 text-muted-foreground hover:bg-primary/10 hover:text-primary cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="rounded-full p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive cursor-pointer"
                      >
      ...
      {/* Mobile Guest Cards (Mobile View) */}
      <div className="space-y-4 md:hidden">
        {filtered.map((g) => (
          <div
            key={g.id}
            className="card-soft p-4 flex flex-col gap-3.5 border border-border/40 bg-card/60 backdrop-blur-md"
          >
              ...
              <div className="flex items-start gap-3 min-w-0">
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h3 className="font-semibold text-foreground text-sm truncate leading-snug">
                      {g.firstName} {g.lastName}
                    </h3>
            ...
            {/* Badges Grid Row */}
            <div className="grid grid-cols-3 gap-2 border-t border-b border-border/30 py-2.5">
            ...
            {/* Address Footer Row */}
            {(g.street || g.city) && (
              <div className="text-[11px] text-muted-foreground bg-muted/40 rounded-xl p-2.5">
                <div className="font-semibold text-foreground text-[10px] uppercase tracking-wider mb-1">
                  Delivery Address
                </div>
                <p className="truncate text-foreground/80">{g.street}</p>
        ...
        {filtered.length === 0 && (
          <div className="card-soft p-12 text-center text-muted-foreground text-sm">
            No guests match your filters.
          </div>
        )}
      </div>
...
function Stat({
  ...
  return (
    <div className="card-soft flex items-center gap-3 p-4">
      <div
        className={`flex h-12 w-12 items-center justify-center rounded-2xl ${ring} ${fg} font-display text-xl font-semibold`}
      >
        {value}
      </div>
...
function Badge({ children, cls }: { children: React.ReactNode; cls: string }) {
  return (
    <span
      className={`inline-block rounded-full px-2 sm:px-2.5 py-0.5 text-[10px] sm:text-xs font-semibold capitalize ${cls}`}
    >
      {children}
    </span>
  );
...
function GuestModal({
  ...
  const [form, setForm] = useState<Partial<Guest>>(
    guest ?? { firstName: "", lastName: "", email: "", phone: "", street: "", city: "", pin: "" },
  );
  return (
    <div
    ...
    >
      <div
        className="w-full max-w-lg rounded-3xl bg-card p-6 shadow-2xl max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button
            onClick={() => onSave(form)}
            disabled={!form.firstName || !form.city}
...
function InviteModal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md rounded-3xl bg-card p-6 shadow-2xl max-h-[90vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-4 flex items-center justify-between">
        ...
        <div className="mt-4 rounded-2xl border border-border bg-background p-4 text-sm">
          ...
          You're invited 🌱 — open your personal memory page from us. Tap to view photos, wishes,
          and RSVP.
        </div>
        <button
          onClick={onSend}
          className="mt-5 w-full rounded-full bg-primary px-5 py-3 font-medium text-primary-foreground hover:opacity-90"
        >
          Send via {channel}
        </button>
      </div>
